...
public class AntiDiagControlOption implements Option {
  ...
  private static final long BEST_POSITIONS = 4398048608256L;
  ...
  public boolean isBoardInInitiationSet(Board board, PLAYER_COLOR playerColor) {
    ...
    if(isWhite){
      ...
      return (board.getBlack() & ANTIDIAG) == 0;
    }
  }
  ...
  public Move getBestMove(Board board, List<Move> possibleMoves) {
    ...
    for (Move move : possibleMoves) {
      ...
      if((isWhite && (clone.getWhite() & ANTIDIAG) != 0) || (!isWhite && (clone.getBlack() & ANTIDIAG) != 0)){
        movesToTakeDiag.add(move);
        if((isWhite && (clone.getWhite() & BEST_POSITIONS) != 0) || (!isWhite && (clone.getBlack() & BEST_POSITIONS) != 0)){
