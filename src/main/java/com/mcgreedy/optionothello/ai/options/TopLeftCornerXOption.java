...
public class TopLeftCornerXOption implements Option {
  ...
  public boolean isBoardInInitiationSet(Board board, PLAYER_COLOR playerColor) {
    if (playerColor == PLAYER_COLOR.WHITE) {
      return (board.getBlack() & TOPLEFT_X_FIELD) != 0 && (board.getWhite() & board.getBlack() & TOPLEFT_CORNER) == 0;
    } else {
      return (board.getWhite() & TOPLEFT_X_FIELD) != 0 && (board.getBlack() & board.getWhite() & TOPLEFT_CORNER) == 0;
    }
  }
  ...
  public boolean shouldTerminate(Board board, PLAYER_COLOR playerColor) {
    if (playerColor == PLAYER_COLOR.WHITE) {
      return (board.getWhite() & TOPLEFT_CORNER) != 0;
    } else {
      return (board.getBlack() & TOPLEFT_CORNER) != 0;
    }
  }
